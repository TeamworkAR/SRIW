...
public class AccessibleButton : UAP_BaseElement
{
	...
	protected override void OnInteract()
	{
		...
		IPointerClickHandler pointerClick = GetPointerClick();
		if (button != null)
		{
		...
		else if (pointerClick != null)
		{
			
			var pointer = new PointerEventData(EventSystem.current); // pointer event for Execute
			pointerClick.OnPointerClick(pointer);
			return;
		}
	...
	private IPointerClickHandler GetPointerClick()
	{
		IPointerClickHandler refButton = null;
		if (m_ReferenceElement != null && m_ReferenceElement.activeInHierarchy)
			refButton = m_ReferenceElement.GetComponent<IPointerClickHandler>();
		if (refButton == null && gameObject.activeInHierarchy)
			refButton = gameObject.GetComponent<IPointerClickHandler>();

		return refButton;
	}
	private IPointerEnterHandler GetHighlight()
	{
		IPointerEnterHandler refButton = null;
		if (m_ReferenceElement != null && m_ReferenceElement.activeInHierarchy)
			refButton = m_ReferenceElement.GetComponent<IPointerEnterHandler>();
		if (refButton == null && gameObject.activeInHierarchy)
			refButton = gameObject.GetComponent<IPointerEnterHandler>();

		return refButton;
	}
	private IPointerExitHandler GetEndighlight()
	{
		IPointerExitHandler refButton = null;
		if (m_ReferenceElement != null && m_ReferenceElement.activeInHierarchy)
			refButton = m_ReferenceElement.GetComponent<IPointerExitHandler>();
		if (refButton == null && gameObject.activeInHierarchy)
			refButton = gameObject.GetComponent<IPointerExitHandler>();
	...
	public override bool IsInteractable()
	{
		...
		IPointerClickHandler pointerClickHandler = GetPointerClick();
	...
	protected override void OnHoverHighlight(bool enable)
	{
		...
		{
			IPointerEnterHandler pointerEnterHandler = GetHighlight();
			IPointerExitHandler pointerExitHandler = GetEndighlight();
			
			var pointer = new PointerEventData(EventSystem.current); // pointer event for Execute
			if (enable)
				pointerEnterHandler?.OnPointerEnter(pointer);
			else
				pointerExitHandler?.OnPointerExit(pointer);
		}
#endif
	}
