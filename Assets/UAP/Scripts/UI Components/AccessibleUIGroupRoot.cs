...
public class AccessibleUIGroupRoot : MonoBehaviour
{
	...
	{
		for (int x = m_AllElements.Count - 1; x >= 0; x--)
		{
			if (m_AllElements[x].m_Object == item)
			{
				if (m_CurrentItemIndex >= x)
				{
					m_CurrentItemIndex = Mathf.Max(m_CurrentItemIndex - 1, 0);
				}
				var element = m_AllElements[x];
	...
	public int RefreshContainer()
	{
		...
		else if (m_CurrentItemIndex >= m_AllElements.Count)
		{
	...
	public bool SelectItem(UAP_BaseElement element, bool forceRepeatItem = false, bool selectWithoutReading = false)
	{
		for (int i = 0; i < m_AllElements.Count; ++i)
		{
			...
			if (accessElement.m_Object == element)
			{
				...
				return UAP_AccessibilityManager.MakeActiveContainer(this, forceRepeatItem, selectWithoutReading);
			}
		}
		return false;
	}
