...
public abstract class UAP_BaseElement : MonoBehaviour
{
	...
	*/
	...
	private async void WaitAFrameAsync()
	{
		await Task.Yield();
	...
	static public string FilterText(string text)
	{
		...
		RemoveSubsting(ref text, "<I>");
		RemoveSubsting(ref text, "</I>");
		RemoveSubsting(ref text, "<indent=2%>");
		RemoveSubsting(ref text, "</indent>");
		RemoveSubsting(ref text, "<indent=1em>");
		RemoveSubsting(ref text, "\\u2022");
		RemoveSubsting(ref text, "<align=\"left\">");
		RemoveSubsting(ref text, "<size=200%>");
		RemoveSubsting(ref text, "</size>");
	...
	{
		if(substring == null || string.IsNullOrEmpty(text))
			return;
	...
	public bool SelectItem(bool forceRepeatItem = false, bool selectWithoutReading = false)
	{
		if (!IsElementActive())
		{
		...
		return SelectItem_Internal(forceRepeatItem, selectWithoutReading);
	}

	public void Select()
	{
		string activeText = UAP_AccessibilityManager.ActiveText;
		//Don't reselect the current object if it's already saying the same text
		if(activeText != null && UAP_AccessibilityManager.IsSpeaking() && this.gameObject == UAP_AccessibilityManager.GetCurrentFocusObject() &&
		   activeText.Equals(GetMainText()))
			return;
		SelectItem(true);
	}
	public void Select(bool selectWithoutAudio)
	{
		SelectItem(true, selectWithoutAudio);
	}

	//////////////////////////////////////////////////////////////////////////

	private bool SelectItem_Internal(bool forceRepeatItem, bool selectWithoutReading = false)
	{
		if (AUIContainer == null)
		{
		...
		return AUIContainer.SelectItem(this, forceRepeatItem, selectWithoutReading);
	}
