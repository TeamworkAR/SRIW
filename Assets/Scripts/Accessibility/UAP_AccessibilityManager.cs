...
public class UAP_AccessibilityManager : MonoBehaviour
{
	...
	public const string k_ENABLE_ACCESSABILITY = "CommonTable/k_ENABLED_ACCESSIBILITY";
	public const string k_DISABLE_ACCESSABILITY = "CommonTable/k_DISABLE_ACCESSIBILITY";
	public const string k_ELEMENT_DISABLED = "CommonTable/k_ELEMENT_DISABLE";
	public const string k_ELEMENT_BUTTON = "CommonTable/k_ELEMENT_BUTTON";
	public const string k_ELEMENT_SLIDER = "CommonTable/k_ELEMENT_SLIDER";
	public const string k_ELEMENT_TOGGLE = "CommonTable/k_ELEMENT_TOGGLE";
	public const string k_ELEMENT_TEXTEDIT = "CommonTable/k_ELEMENT_TEXTEDIT";
	public const string k_ELEMENT_DROPDOWN = "CommonTable/k_ELEMENT_DROPDOWN";
	
	public const string k_ELEMENT_HINT_DESKTOP_BUTTON = "CommonTable/k_ELEMENT_HINT_DESKTOP_BUTTON";
	public const string k_ELEMENT_HINT_MOBILE_BUTTON = "CommonTable/k_ELEMENT_HINT_MOBILE_BUTTON";
	
	public const string k_ELEMENT_HINT_DESKTOP_DROPDOWN = "CommonTable/k_ELEMENT_HINT_DESKTOP_DROPDOWN";
	public const string k_ELEMENT_HINT_MOBILE_DROPDOWN = "CommonTable/k_ELEMENT_HINT_MOBILE_DROPDOWN";
	
	public const string k_ELEMENT_DROPDOWN_ITEM_SELECTED = "CommonTable/k_DROPDOWN_ITEM_SELECTED";
	public const string k_ELEMENT_DROPDOWN_ITEM_INDEX = "CommonTable/k_DROPDOWN_ITEM_INDEX";
	...
	public static string ActiveText => instance.m_AudioQueue?.ActiveEntry?.m_TTS_Text;
	...
	{
		//This was killer. Was clearing on scenes loaded additively.
	...
	public static bool ShouldUseBuiltInKeyboard()
	{
		...
		if (!Application.isMobilePlatform && !MobileBrowserDetection.IsMobileBrowser)
			return false;
	...
	void ReadDisabledState()
	{
			...
			m_CurrentItem.m_Type == AccessibleUIGroupRoot.EUIElement.EDropDown)
		{
			if (!m_CurrentItem.m_Object.IsInteractable())
			{
				...
				SayAudio(m_DisabledAsAudio, Localize_Internal(k_ELEMENT_DISABLED), UAP_AudioQueue.EAudioType.Element_Hint, m_CurrentItem.m_Object.m_AllowVoiceOver);
			}
		}
	}
	...
	void ReadType()
	{
		...
		switch (m_CurrentItem.m_Type)
		{
			case AccessibleUIGroupRoot.EUIElement.EButton:
				SayAudio(m_ButtonAsAudio, Localize_Internal(k_ELEMENT_BUTTON), UAP_AudioQueue.EAudioType.Element_Type, m_CurrentItem.m_Object.m_AllowVoiceOver);
				//if (!m_CurrentItem.m_Object.IsInteractable())
				//  SayAudio(m_DisabledAsAudio, "Disabled", UAP_AudioQueue.EAudioType.Element_Hint);
				break;

			case AccessibleUIGroupRoot.EUIElement.ESlider:
				SayAudio(m_SliderAsAudio, Localize_Internal(k_ELEMENT_SLIDER), UAP_AudioQueue.EAudioType.Element_Type, m_CurrentItem.m_Object.m_AllowVoiceOver);
				//if (!m_CurrentItem.m_Object.IsInteractable())
				//  SayAudio(m_DisabledAsAudio, "Disabled", UAP_AudioQueue.EAudioType.Element_Hint);
				break;

			case AccessibleUIGroupRoot.EUIElement.EToggle:
				SayAudio(m_ToggleAsAudio, Localize_Internal(k_ELEMENT_TOGGLE), UAP_AudioQueue.EAudioType.Element_Type, m_CurrentItem.m_Object.m_AllowVoiceOver);
				//if (!m_CurrentItem.m_Object.IsInteractable())
				//  SayAudio(m_DisabledAsAudio, "Disabled", UAP_AudioQueue.EAudioType.Element_Hint);
				break;

			case AccessibleUIGroupRoot.EUIElement.ETextEdit:
				SayAudio(m_TextEditAsAudio, Localize_Internal(k_ELEMENT_TEXTEDIT), UAP_AudioQueue.EAudioType.Element_Type, m_CurrentItem.m_Object.m_AllowVoiceOver);
				//if (!m_CurrentItem.m_Object.IsInteractable())
				//  SayAudio(m_DisabledAsAudio, "Disabled", UAP_AudioQueue.EAudioType.Element_Hint);
				break;

			case AccessibleUIGroupRoot.EUIElement.EDropDown:
				SayAudio(m_DropDownAsAudio, Localize_Internal(k_ELEMENT_DROPDOWN), UAP_AudioQueue.EAudioType.Element_Type, m_CurrentItem.m_Object.m_AllowVoiceOver);
				//if (!m_CurrentItem.m_Object.IsInteractable())
				//  SayAudio(m_DisabledAsAudio, "Disabled", UAP_AudioQueue.EAudioType.Element_Hint);
				break;
		};
	...
	void ReadHint()
	{
		...
		else
		{
			switch (m_CurrentItem.m_Type)
			{
				case AccessibleUIGroupRoot.EUIElement.EButton:
					if (Application.isMobilePlatform || MobileBrowserDetection.IsMobileBrowser)
						SayAudio(null, Localize_Internal(k_ELEMENT_HINT_MOBILE_BUTTON), UAP_AudioQueue.EAudioType.Element_Hint, m_CurrentItem.m_Object.m_AllowVoiceOver);
					else
						SayAudio(null, Localize_Internal(k_ELEMENT_HINT_DESKTOP_BUTTON), UAP_AudioQueue.EAudioType.Element_Hint, m_CurrentItem.m_Object.m_AllowVoiceOver);
					break;
				...
				case AccessibleUIGroupRoot.EUIElement.EDropDown:
					if (Application.isMobilePlatform || MobileBrowserDetection.IsMobileBrowser)
						SayAudio(null, Localize_Internal(k_ELEMENT_HINT_MOBILE_DROPDOWN), UAP_AudioQueue.EAudioType.Element_Hint, m_CurrentItem.m_Object.m_AllowVoiceOver);
					else
						SayAudio(null, Localize_Internal(k_ELEMENT_HINT_DESKTOP_DROPDOWN), UAP_AudioQueue.EAudioType.Element_Hint, m_CurrentItem.m_Object.m_AllowVoiceOver);
					break;
	...
	{
		Debug.Log("Accessibility Enabled");
		EnableAccessibility(true);
	}
	public void DisableAccessibility()
	{
		Debug.Log("Accessibility Disabled");
		EnableAccessibility(false);
	}

	public void WebToggleAccessibility()
	{
		ToggleAccessibility();
	}
	...
	public static void EnableAccessibility(bool enable, bool readNotification = false)
	{
		...
		if (enable)
		{
			...
			if (readNotification)
				instance.Say_Internal(Localize_Internal(k_ENABLE_ACCESSABILITY), false, true, UAP_AudioQueue.EInterrupt.All);
		...
		else // if turn off
		{
			...
			if (readNotification)
				instance.Say_Internal(Localize_Internal(k_DISABLE_ACCESSABILITY), false, true, UAP_AudioQueue.EInterrupt.All);
	...
	private void ToggleAccessibility_Internal()
	{
		...
		if (wasEnabled)
			Say_Internal(Localize_Internal(k_DISABLE_ACCESSABILITY), false, true, UAP_AudioQueue.EInterrupt.All);
	}
	...
	void UpdateDoubleTapDetection()
	{
		...
		int touchCount = GetTouchCount();
		//This is used to check for touch input on low performance machines
		//Input.GetMouseButtonDown(0) wont register if performance is low
		bool touchStart = false;
		if (Input.touches.Length > 0)
		{
			touchStart = Input.touches[0].phase == TouchPhase.Began;
		}
		// This will work on touch as well
		if ((Input.GetMouseButtonDown(0) || touchStart) && touchCount == 1)
		{
	...
	private void LeaveFocussedItem()
	{
		...
		else if (m_CurrentItem.m_Type == AccessibleUIGroupRoot.EUIElement.EDropDown)
		{
			...
			// "TT selected"
			string toSay = Localize_Internal(k_ELEMENT_DROPDOWN_ITEM_SELECTED);
			string valueText = m_CurrentItem.m_Object.GetCurrentValueAsText();
	...
	private void ReadDropdownItemIndex(ref AccessibleUIGroupRoot.Accessible_UIElement item)
	{
		...
		// "Item XX of YY"
		string toSay = Localize_Internal(k_ELEMENT_DROPDOWN_ITEM_INDEX);
	...
	static public bool MakeActiveContainer(AccessibleUIGroupRoot container, bool forceRepeatActiveItem = false, bool selectWithoutReading = false)
	{
		if (instance == null)
			return false;
		...
		instance.UpdateCurrentItem(UAP_BaseElement.EHighlightSource.Internal, !selectWithoutReading);

		// Read only if the item was already selected (unless forceRepeat is on)
		if (!selectWithoutReading && (!alreadySelected || forceRepeatActiveItem))
			ReadItem(instance.m_CurrentItem);
	...
	{
		//As a shim, we are splitting the key as a path that is TableKey/LocalizationKey
		string[] keys = key.Split('/');
		if (keys.Length != 2)
		{
			Debug.LogWarning($"Accessibility key is not localized {key}");
			return key;
		}
		...
		return LocalizationManager.Instance.GetLocalizedValue(keys[0], keys[1]);
	}
